...
describe('Test modal dialogs functionality', () => {
  ...
  it('Check small modal opens with its content and closes', () => {
    ...
    modalDialogsPOM.openSmallModal().checkSmallModalContent().closeSmallModal();
  });

  it('Check large modal opens with its content and closes', () => {
    homepagePOM.clickCategoryCard('Alerts, Frame & Windows').clickMenuItem('Modal Dialogs');
    modalDialogsPOM.openLargeModal().checkLargeModalContent().closeLargeModal();
  });

  it('Check small modal closes with the X icon', () => {
    homepagePOM.clickCategoryCard('Alerts, Frame & Windows').clickMenuItem('Modal Dialogs');
    modalDialogsPOM.openSmallModal().checkSmallModalContent().closeModalWithIcon();
  });

  it('Check large modal closes with the X icon', () => {
    homepagePOM.clickCategoryCard('Alerts, Frame & Windows').clickMenuItem('Modal Dialogs');
    modalDialogsPOM.openLargeModal().checkLargeModalContent().closeModalWithIcon();
  });
});
